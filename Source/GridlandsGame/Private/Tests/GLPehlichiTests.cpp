...
namespace GLPehlichiTests
{
	...
	const TCHAR* Buried = TEXT("placement.origin.glitch_buried_signal");
	const FName ScanCapability(TEXT("capability.pehlichi.scan"));
}
...
bool FGLNoPlayerRepair::RunTest(const FString& Parameters)
{
	...
	AGLGlitch* Glitch = Scene.Glitch(Lamp);
	...
	TestEqual(TEXT("spawned Latent"), Scene.State(Lamp), EGLGlitchState::Latent);
...
{
	FPehlichiScene Scene;
	const FVector LampAt = Scene.Glitch(Lamp)->GetActorLocation();
	Scene.Place(Scene.Pehlichi, LampAt + FVector(300, 0, 0));
	const FGLScanResult Result = Scene.Pehlichi->GetScan()->Scan();
	TestEqual(TEXT("level-1 scan reveals the nearby lamp"), Scene.State(Lamp), EGLGlitchState::Detected);
	TestTrue(TEXT("and reports it"), Result.NewlyRevealed == 1 && Result.Findings.Num() == 1);
	TestEqual(TEXT("the transformer is out of range"), Scene.State(Transformer), EGLGlitchState::Latent);
	TestTrue(TEXT("revealed glitches become visible"), Cast<UPrimitiveComponent>(Scene.Glitch(Lamp)->GetRootComponent())->IsVisible());

	Scene.Place(Scene.Pehlichi, Scene.Glitch(Buried)->GetActorLocation() + FVector(100, 0, 0));
	Scene.Pehlichi->GetScan()->Scan();
	TestEqual(TEXT("a level-2 glitch stays hidden from a level-1 scan, even point-blank"), Scene.State(Buried), EGLGlitchState::Latent);
...
bool FGLLoopSteps5To12::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("5: the glitch is not visible at first"), Scene.State(Lamp), EGLGlitchState::Latent);
	...
	TestEqual(TEXT("7: Pehlichi reveals it"), Scene.State(Lamp), EGLGlitchState::Detected);

	Scene.Run(0.5);
	TestEqual(TEXT("the blocker is still there: requirements unmet"), Scene.State(Lamp), EGLGlitchState::Detected);
	...
	Scene.Run(0.5);
	TestEqual(TEXT("8: Zenny salvaged the blocker; the glitch is repairable"), Scene.State(Lamp), EGLGlitchState::Repairable);
	...
	Scene.Run(1.5);
	TestEqual(TEXT("10: Pehlichi walked over and is repairing"), Scene.State(Lamp), EGLGlitchState::Repairing);
	...
	Scene.Run(6.0);
	TestEqual(TEXT("11: repaired"), Scene.State(Lamp), EGLGlitchState::Repaired);
	TestTrue(TEXT("Event.Glitch.Repaired"), Scene.Events.Contains(TEXT("Event.Glitch.Repaired:glitch.home.flicker_lamp")));
	TestEqual(TEXT("12: Pehlichi's scan permanently improved to level 2"), Scene.Pehlichi->GetCapabilities()->Level(ScanCapability), 2);

	// The upgrade matters: the level-2 glitch is now findable.
	Scene.Place(Scene.Pehlichi, Scene.Glitch(Buried)->GetActorLocation() + FVector(100, 0, 0));
	Scene.Pehlichi->GetScan()->Scan();
	TestEqual(TEXT("the upgraded scan reveals the buried signal"), Scene.State(Buried), EGLGlitchState::Detected);
	return true;
}
...
{
	FPehlichiScene Scene;
	const FVector At = Scene.Glitch(Transformer)->GetActorLocation();
	...
	Scene.Run(0.5);
	TestEqual(TEXT("without a fuse it is not repairable"), Scene.State(Transformer), EGLGlitchState::Detected);

	Scene.Inventory->AddItem(TEXT("item.part.fuse"), 1);
	Scene.Run(0.5);
	TestEqual(TEXT("carrying the fuse makes it repairable"), Scene.State(Transformer), EGLGlitchState::Repairable);
	Commands->Issue(TEXT("Command.Pehlichi.Repair"), Scene.Zenny);
	Scene.Run(4.0); // ~1 s to walk over, then ~3 s of repair
	TestEqual(TEXT("repairing"), Scene.State(Transformer), EGLGlitchState::Repairing);
	TestEqual(TEXT("Pehlichi took the fuse at repair start"), Scene.Inventory->CountOf(TEXT("item.part.fuse")), 0);

	const double Before = Scene.Glitch(Transformer)->GetGlitch()->GetProgressSeconds();
	Commands->Issue(TEXT("Command.Pehlichi.Follow"), Scene.Zenny);
	TestEqual(TEXT("recalling Pehlichi interrupts the repair"), Scene.State(Transformer), EGLGlitchState::Interrupted);
	Scene.Run(1.0);
	TestEqual(TEXT("the delivered fuse still counts: it does not fall back to Detected"), Scene.State(Transformer), EGLGlitchState::Interrupted);
	TestEqual(TEXT("KeepProgress keeps progress"), Scene.Glitch(Transformer)->GetGlitch()->GetProgressSeconds(), Before);

	TestEqual(TEXT("resume"), Commands->Issue(TEXT("Command.Pehlichi.Repair"), Scene.Zenny), EGLCommandRejection::None);
	Scene.Run(10.0);
	TestEqual(TEXT("completes after resuming"), Scene.State(Transformer), EGLGlitchState::Repaired);
