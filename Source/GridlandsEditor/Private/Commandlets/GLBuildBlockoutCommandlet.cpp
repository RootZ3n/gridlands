...
{
	const TCHAR* GridPackage = TEXT("/Game/Gridlands/Maps/L_Grid");
	const TCHAR* OriginPackage = TEXT("/Game/Gridlands/Maps/L_Origin");
	const TCHAR* DinerLotsPackage = TEXT("/Game/Gridlands/Maps/L_DinerLots");
	constexpr double M = 100.0; // cm per metre

	struct FBuilder
	{
		UWorld* World;
		FString AnchorPrefix = TEXT("anchor.origin");
		...
		void Anchor(AActor* Actor, const TCHAR* Name)
		{
			...
			Anchor->AnchorId = FName(FString::Printf(TEXT("%s.%s"), *AnchorPrefix, Name));
...
namespace GLBlockout
{
	/** Replaces a map package with a freshly generated world (the generator is the map's source). */
	bool WriteMap(const TCHAR* PackageName, const TCHAR* WorldName, const TCHAR* AnchorPrefix, TFunctionRef<void(UWorld*, FBuilder&)> Populate)
	{
		// Start from nothing. (Loading the old package and creating a second world inside it crashed
		// with "Cannot generate unique name for 'WorldSettings'".)
		const FString Filename = FPackageName::LongPackageNameToFilename(PackageName, FPackageName::GetMapPackageExtension());
		if (IFileManager::Get().FileExists(*Filename) && !IFileManager::Get().Delete(*Filename))
		{
			UE_LOG(LogGridlandsEditor, Error, TEXT("BuildBlockout: could not replace %s"), *Filename);
			return false;
		}
		UPackage* Package = CreatePackage(PackageName);
		UWorld* World = UWorld::CreateWorld(EWorldType::Inactive, false, WorldName, Package);
		World->SetFlags(RF_Public | RF_Standalone);
		FBuilder B{ World, AnchorPrefix };
...
int32 UGLBuildBlockoutCommandlet::Main(const FString& Params)
{
	...
	// The second cell (P3 architecture proof, not Zone 2): a few 1950s lots, cell-local coordinates.
	const bool bLots = WriteMap(DinerLotsPackage, TEXT("L_DinerLots"), TEXT("anchor.diner_lots"), [](UWorld* World, FBuilder& B)
	{
		B.Box(TEXT("Lots_Street"), FVector(0, 0, 0.02 * M), FVector(256, 10, 0.04));
		B.Box(TEXT("Lots_Diner_Starlite"), FVector(20 * M, -22 * M, 2.25 * M), FVector(18, 8, 4.5));
		B.Shape(B.Cylinder, TEXT("Lots_Diner_Starlite_Cap"), FVector(29 * M, -22 * M, 2.25 * M), FVector(8, 8, 4.5));
		B.Shape(B.Cylinder, TEXT("Lots_Sign_Pole"), FVector(8 * M, -14 * M, 5 * M), FVector(0.4, 0.4, 10));
		B.Box(TEXT("Lots_Sign"), FVector(8 * M, -14 * M, 10.5 * M), FVector(5, 0.4, 2));
		for (int32 Car = 0; Car < 4; ++Car)
		{
			B.Box(*FString::Printf(TEXT("Lots_Car_%d"), Car + 1), FVector((-20 + 9 * Car) * M, 14 * M, 0.7 * M), FVector(4.6, 1.9, 1.4), 90.0);
		}
		B.Box(TEXT("Lots_Motel"), FVector(-45 * M, -30 * M, 2 * M), FVector(30, 9, 4));
	});

	// The persistent Grid map: sky, sun, the player start and the game mode. It owns no cell;
	// UGLGridSubsystem streams cells (their level and their runtime layer) in and out.
	const bool bGrid = WriteMap(GridPackage, TEXT("L_Grid"), TEXT("anchor.grid"), [](UWorld* World, FBuilder& B)
	{
