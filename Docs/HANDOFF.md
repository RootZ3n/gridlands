...
- **Change the save format:** add the field to `FGLWorldSave` and capture/apply it in
  `UGLSaveSubsystem`. If an existing field changes meaning, bump `CurrentVersion` and add the
  migration in `GLSaveCodec::FromJson` with a test. Never save a derived value (S-1).
- **Start a fresh world:** delete `Saved/SaveGames/Gridlands/world.json` or launch with `-GLNewWorld`.
